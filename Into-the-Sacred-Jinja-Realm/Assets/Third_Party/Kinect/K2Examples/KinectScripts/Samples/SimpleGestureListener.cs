	                              ...
	                              float progress, KinectInterop.JointType joint, Vector3 screenPos)
	{
        ...
        switch (gesture)
        {
			...
			case KinectGestures.Gestures.SwipeLeft:
				gestureInfo.text = "Swipe left - progress: " + (progress * 100) + "%";
				Debug.Log("Swipe left - progress: " + (progress * 100) + "%");
				break;
			case KinectGestures.Gestures.SwipeRight:
				gestureInfo.text = "Swipe right - progress: " + (progress * 100) + "%";
				Debug.Log("Swipe right - progress: " + (progress * 100) + "%");
				break;
			...
			case KinectGestures.Gestures.RaisedLeftHorizontalRightHand:
			 	if(progress > 0.5f && gestureInfo != null)
				{
					string sGestureText = string.Format("{0} - progress: {1:F0}%", gesture, progress * 100);
					gestureInfo.text = sGestureText;
					Debug.Log("RaisedLeftHorizontalRightHand - progress: " + (progress * 100) + "%");
					progressDisplayed = true;
					progressGestureTime = Time.realtimeSinceStartup;
				}	
				break;
			case KinectGestures.Gestures.RaisedRightHorizontalLeftHand:
				if(progress > 0.5f && gestureInfo != null)
				{
					string sGestureText = string.Format("{0} - progress: {1:F0}%", gesture, progress * 100);
					gestureInfo.text = sGestureText;
					Debug.Log("RaisedRightHorizontalLeftHand - progress: " + (progress * 100) + "%");
					progressDisplayed = true;
					progressGestureTime = Time.realtimeSinceStartup;
				}	
				break;
				case KinectGestures.Gestures.RaisedLeftRaisedRightHand:
				if(progress > 0.5f && gestureInfo != null)
				{
					string sGestureText = string.Format("{0} - progress: {1:F0}%", gesture, progress * 100);
					gestureInfo.text = sGestureText;
					Debug.Log("RaisedLeftRaisedRightHand - progress: " + (progress * 100) + "%");
					progressDisplayed = true;
					progressGestureTime = Time.realtimeSinceStartup;
				}	
				break;
