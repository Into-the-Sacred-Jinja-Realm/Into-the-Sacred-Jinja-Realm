...
//		LeftHandCursor,
		...
		RaisedLeftRaisedRightHand,  //by Nariaki S.
...
//			return new int[0];
	...
	protected float GetJointAngle(Vector3 a, Vector3 b, Vector3 c)
	{
		Vector3 ab = a - b;
		Vector3 cb = c - b;
	...
	public virtual void CheckForGesture(long userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
		...
		switch (gestureData.gesture)
		{
			...
			case Gestures.RaiseLeftHand:
				...
				break;
			// check for HandTogerther
			case Gestures.HandTogerther:
				switch (gestureData.state)
				{
					case 0:  // gesture detection
						if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] &&
							Vector3.Distance(jointsPos[leftHandIndex], jointsPos[rightHandIndex]) <= 0.15f)
						{
							SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
						}
						break;
			...
			case Gestures.HandClap:
				...
				{
				case 0: // gesture detection
					if (Gestures_Listener.Instance != null && Gestures_Listener.Instance.IsHandClapBlocked())
					break;
					if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex])
						{
							float handsDist = Vector3.Distance(jointsPos[leftHandIndex], jointsPos[rightHandIndex]);
							// Check if both hands are close and moving faster than the threshold
							Vector3 leftHandVel = (jointsPos[leftHandIndex] - gestureData.tagVector) / Mathf.Max(0.001f, timestamp - gestureData.timestamp);
							Vector3 rightHandVel = (jointsPos[rightHandIndex] - gestureData.tagVector2) / Mathf.Max(0.001f, timestamp - gestureData.timestamp);
							float relVel = Vector3.Dot(leftHandVel - rightHandVel, (jointsPos[rightHandIndex] - jointsPos[leftHandIndex]).normalized);

							if (handsDist <= 0.15f && relVel > 1.0f)
							{
								SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
							}
							// record the hand positions
							gestureData.tagVector = jointsPos[leftHandIndex];
							gestureData.tagVector2 = jointsPos[rightHandIndex];
							gestureData.timestamp = timestamp;
						}
				break;
				case 1: // gesture complete
			...
			case Gestures.HandDownRight:
				switch (gestureData.state)
				{
					case 0: // gesture detection
						...
						break;
					case 1: // gesture complete
						bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[leftShoulderIndex] &&
							(jointsPos[rightHandIndex].y - jointsPos[leftShoulderIndex].y) < -0.1f;
						Vector3 jointPos = jointsPos[gestureData.joint];
						CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, KinectInterop.Constants.PoseCompleteDuration);
						break;
			...
			case Gestures.HandDownLeft:
				switch (gestureData.state)
				{
					case 0: // gesture detection
						...
						break;
					case 1: // gesture complete
						bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[rightShoulderIndex] &&
							(jointsPos[leftHandIndex].y - jointsPos[rightShoulderIndex].y) < -0.1f;
						Vector3 jointPos = jointsPos[gestureData.joint];
						CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, KinectInterop.Constants.PoseCompleteDuration);
						break;
			...
			case Gestures.BothHandsDown:
				switch (gestureData.state)
				{
					case 0: // gesture detection
						if (
							jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] &&
							jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] &&
			...
			// check for RaisedLeftRaisedRightHand
			case Gestures.RaisedLeftRaisedRightHand:
				switch (gestureData.state)
				{
					case 0: // gesture detection
						if (jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] && // check if left hand is raised vertically
							(jointsPos[leftHandIndex].y - jointsPos[leftShoulderIndex].y) > 0.25f && // ensure left hand is higher than the left shoulder
							Mathf.Abs(jointsPos[leftHandIndex].z - jointsPos[leftShoulderIndex].z) < 0.45f && // ensure left hand is vertically straight enough
							Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[leftShoulderIndex].x) < 0.3f && // ensure left hand is vertically straight enough
							jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && // check if right hand is raised vertically
							(jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) > 0.25f && // ensure right hand is higher than the right shoulder
							Mathf.Abs(jointsPos[rightHandIndex].z - jointsPos[rightShoulderIndex].z) < 0.45f && // ensure right hand is vertically straight enough
							Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightShoulderIndex].x) < 0.3f) // ensure right hand is vertically straight enough
						{
							// set the starting point of the gesture
							SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
						}
						break;

					case 1: // gesture completion phase
						bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] && // check if left hand is raised vertically
							(jointsPos[leftHandIndex].y - jointsPos[leftShoulderIndex].y) > 0.25f && // ensure left hand is higher than the left shoulder
							Mathf.Abs(jointsPos[leftHandIndex].z - jointsPos[leftShoulderIndex].z) < 0.45f && // ensure left hand is vertically straight enough
							Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[leftShoulderIndex].x) < 0.3f && // ensure left hand is vertically straight enough
							jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && // check if right hand is raised vertically
				...
				switch (gestureData.state)
				{
					case 0:  // gesture detection
						if (jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] && // check left hand is straight up 
						   ...
						   Mathf.Abs(jointsPos[leftHandIndex].z - jointsPos[leftShoulderIndex].z) < 0.45f &&   // ensure hand is vertical straight enough 
						   Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[leftShoulderIndex].x) < 0.3f &&   // ensure hand is vertical straight enough                             
			...
			case Gestures.Bow:
				...
				int spine = (int)KinectInterop.JointType.SpineMid;

				switch (gestureData.state)
				{
					case 0: // 偵測階段
						if (jointsTracked[head] && jointsTracked[neck] && jointsTracked[spine])
						{
							float bowAngle = GetJointAngle(jointsPos[head], jointsPos[neck], jointsPos[spine]);
							float zDiff = jointsPos[head].z - jointsPos[spine].z;

							Debug.LogFormat("Bow: angle={0}, zDiff={1}", bowAngle, zDiff);

							if (bowAngle > 25f && zDiff < -0.05f)
							{
								// SetGestureJoint(ref gestureData, timestamp, head, jointsPos[head]);
								SetGestureJoint(ref gestureData, timestamp, spine, jointsPos[spine]);
							}
						}
						break;
					case 1:
						bool isInPose = jointsTracked[head] && jointsTracked[neck] && jointsTracked[spine] &&
						GetJointAngle(jointsPos[head], jointsPos[neck], jointsPos[spine]) > 40f &&
						(jointsPos[head].z - jointsPos[spine].z) < -0.1f;
						...
						if (isInPose)
							Debug.Log("Bow pose is inPose == true");
						break;
				}
				break;
			//			// check for Click
